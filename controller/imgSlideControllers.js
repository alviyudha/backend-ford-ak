...
export const getImgSlide = async (req,res) =>{
    try {
        const response = await prisma.img_slide_home.findMany()
...
export const getImgSlideById = async (req,res) =>{
    try {
        const response = await prisma.img_slide_home.findUnique({
            where:{
...
export const createImgSlide = async (req, res) => {
    ...
    const imgUrl = `${req.protocol}://${req.get("host")}/api/image-slide/${imgName}`;
    ...
    try {
        const newImgSlide = await prisma.img_slide_home.create({
            data: {
                image: imgName,
                urlImage: imgUrl  
            }
...
export const updateImgSlide = async (req, res) => {
    ...
    try {
        const currentData = await prisma.img_slide_home.findUnique({
        ...
        if (newImage) {
            ...
            updatedData.urlImage = `${req.protocol}://${req.get("host")}/api/image-slide/${newImage.filename}`;  // Menyimpan URL baru

            safeDelete(`./public/img-slide/${currentData.image}`);  // Menghapus gambar lama
        }

        const updateImage = await prisma.img_slide_home.update({
            where: { id: parseInt(id) },
            data: updatedData
        });
...
export const deleteImgSlide = async (req,res) =>{
    try {
        const imageSlide = await prisma.img_slide_home.findUnique({
            where: {
                id: Number(req.params.id)
            }
        });
        ...
        await prisma.img_slide_home.delete({
            where: {
                id: Number(req.params.id)
            }
        });
